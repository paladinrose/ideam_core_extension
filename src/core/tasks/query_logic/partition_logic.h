#ifndef IDEAM_CORE_PARTITION_LOGIC_H
#define IDEAM_CORE_PARTITION_LOGIC_H
 ...
 * If N=1, compiles to a branchless single-check kernel.
 * If N>1, compiles into a fully unrolled loop.
 */
template <typename T, size_t N = 1>
struct PartitionLogic {
    using ValueType       = T;
    ...
    // Compile-time fixed array forces loop unrolling
...
private:
    [[nodiscard]] inline int32_t _map_value(const T& p_val) const noexcept {
        if constexpr (N == 1) {
            // Pure branchless resolution for single mappings
            return (p_val == mappings[0].source_value) ? mappings[0].partition_id : default_partition;
        } else {
            // Compiler unrolls this perfectly due to std::array bounds
...
#endif // IDEAM_CORE_PARTITION_LOGIC_H