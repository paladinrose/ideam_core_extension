...
private:
    ...
    inline bool _evaluate(const T& p_val) const {
        if constexpr (C == Comparison::EQUAL)         return p_val == target_value;
        if constexpr (C == Comparison::NOT_EQUAL)     return p_val != target_value;
        if constexpr (C == Comparison::LESS)          return p_val < target_value;
        if constexpr (C == Comparison::LESS_EQUAL)    return p_val <= target_value;
        if constexpr (C == Comparison::GREATER)       return p_val > target_value;
        if constexpr (C == Comparison::GREATER_EQUAL) return p_val >= target_value;
        if constexpr (std::is_integral_v<T>) {
            if constexpr (C == Comparison::BIT_AND)   return (p_val & target_value) != 0;
            if constexpr (C == Comparison::BIT_OR)    return (p_val | target_value) != 0;
            if constexpr (C == Comparison::BIT_XOR)   return (p_val ^ target_value) != 0;
        }
        return false;
    }

    template <Comparison C, typename T_View>
    inline void _loop_dense(uint64_t* p_bitset, int64_t p_capacity, const T_View& p_view, int64_t& r_element_count) const {
    ...
    void _cull_sparse(MemoryBufferSelectionPOD& r_selection, const T_View& p_view) const {
        ...
        switch (op) {
            ...
            case Comparison::BIT_AND:       _loop_sparse<Comparison::BIT_AND>(indices, r_selection.element_count, p_view, write_ptr); break;
            case Comparison::BIT_OR:        _loop_sparse<Comparison::BIT_OR>(indices, r_selection.element_count, p_view, write_ptr); break;
            case Comparison::BIT_XOR:       _loop_sparse<Comparison::BIT_XOR>(indices, r_selection.element_count, p_view, write_ptr); break;
        }
