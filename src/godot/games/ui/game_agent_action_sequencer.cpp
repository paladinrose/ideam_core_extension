// game_agent_action_sequencer.cpp
...
#include "../../controls/selection_list.h"
...
void GameAgentActionSequencer::_deferred_rebuild() {
    ...
    godot::Array sequence = action->get_sequence();
...
void GameAgentActionSequencer::_generate_step(int step_id) {
    ...
    godot::FoldableContainer* step_drop = memnew(godot::FoldableContainer);
    container->add_child(step_drop);
    step_drop->set_h_size_flags(Control::SIZE_EXPAND_FILL);
    step_drop->fold();
    step_drop->set_title("Step " + godot::String::num_int64(step_id + 1));
...
void GameAgentActionSequencer::_generate_piece_step(int step_id, int piece_step_id, godot::Control* step_container) {
    ...
    godot::Array game_pieces = action->get_game_pieces();
    if (piece_id >= 0 && piece_id < game_pieces.size()) {
        piece = godot::Object::cast_to<GamePiece>(game_pieces[piece_id]);
    }
    
    godot::FoldableContainer* piece_panel_container = memnew(godot::FoldableContainer);
    piece_panel_container->set_title("Game Piece: ");
    step_container->add_child(piece_panel_container);
    piece_panel_container->fold();
    
    SelectionList* piece_selector = memnew(SelectionList);
    piece_panel_container->add_title_bar_control(piece_selector);
    ...
    godot::TypedArray<godot::String> piece_names;
    for (int i = 0; i < game_pieces.size(); ++i) {
        GamePiece* p = godot::Object::cast_to<GamePiece>(game_pieces[i]);
        if (p) piece_names.append(p->get_name());
    }
    
    piece_selector->build_list(piece_names);
    piece_selector->set_selection(piece_id);
    
    piece_selector->connect("selection_changed", godot::Callable(this, "_select_piece").bind(step_id, piece_step_id));
    
    godot::TypedArray<godot::String> piece_actions;
    if (piece) {
        godot::Array actions = piece->get_actions();
        for (int j = 0; j < actions.size(); ++j) {
            GamePieceAction* act = godot::Object::cast_to<GamePieceAction>(actions[j]);
            if (act) piece_actions.append(act->get_name());
        }
    }
...
void GameAgentActionSequencer::_generate_piece_action(int step_id, int piece_step_id, int id, godot::Control* piece_container, godot::TypedArray<godot::String> piece_actions) {
    godot::Array sequence = action->get_sequence();
    godot::Array step = sequence[step_id];
    godot::Array piece_step = step[piece_step_id];
    int action_id = piece_step[id];
    
    SelectionList* action_selector = memnew(SelectionList);
    action_selector->set_h_size_flags(Control::SIZE_EXPAND_FILL);
    piece_container->add_child(action_selector);
    
    action_selector->build_list(piece_actions);
    action_selector->set_selection(action_id);
...
void GameAgentActionSequencer::_update_sequence(godot::String action_name, godot::Array new_sequence) {
    if (undo_redo.get_type() != godot::Variant::NIL) {
        ...
        godot::Array old_sequence = action->get_sequence().duplicate(true);
        ...
        action->set_sequence(new_sequence);
    }
...
void GameAgentActionSequencer::_new_sequence_step() {
    ...
    godot::Array act_seq = action->get_sequence().duplicate(true);
    act_seq.append(new_step);
    
    if (undo_redo.get_type() == godot::Variant::NIL) {
        action->set_sequence(act_seq);
        _rebuild_sequencer();
        return;
    }
...
void GameAgentActionSequencer::_new_action(int step_id, int piece_step_id) {
    godot::Array new_sequence = action->get_sequence().duplicate(true);
